...
import { Loader, AlertCircle, Clock, Search, RefreshCw, Filter, ChevronDown, ChevronUp, Calendar, Sun, Moon } from 'lucide-react';
        ...
        {paginatedData.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 w-full lg:grid-cols-2 gap-6">
            {paginatedData.map((alert, index) => (
              <div
                key={index}
                className="bg-white shadow-lg rounded-xl w-[100%] overflow-hidden border border-gray-100 hover:shadow-xl transition-all"
              >
                {/* Card Header */}
               

<div className="bg-gradient-to-r from-indigo-600 to-purple-600 px-6 py-4 flex items-center">
    {parseInt(alert.timestamp.split(' ')[1].split(':')[0]) === 6 ? (
        <>
            <Moon className="text-white w-6 h-6 mr-2" /> 
            <h2 className="text-xl font-bold text-white">Non Working Hours</h2>
        </>
    ) : (
        <>
            <Sun className="text-white w-6 h-6 mr-2" />
